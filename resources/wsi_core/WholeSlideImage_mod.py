...
class WholeSlideImageOpenslide(tiffslide.TiffSlide):
    def __init__(self, path, with_foreground_mask=False, mask_path = None):
        ...
        self.wsi_mask = None
        if with_foreground_mask and mask_path:
            self.with_foreground_mask = with_foreground_mask
            mask_path = tiffslide.open_slide(mask_path)
            self.wsi_mask = tiffslide.open_slide(mask_path)
    ...
    #     self.contours_tumor = [createContour(coord_list) for coord_list in annotations]
    ...
    #
    # def segmentTissueWithMask(self, seg_level=0, sthresh=20, sthresh_up=255, mthresh=7, close=0, use_otsu=False,
    #                           filter_params={'a_t': 100}, ref_patch_size=512, exclude_ids=[], keep_ids=[]):
    #     """
    #         Generate contours and holes from the provided foreground mask
    #     """
    #
    #     if self.wsi_mask is None:
    #         return ValueError('Foreground Mask is not Provided error!')
    #     img_mask = np.array(self.wsi_mask.read_region((0, 0), seg_level, self.level_dim[seg_level]))[
    #         ..., 0]  # only take the red channel as mask in a RGBA image
    #     scale_img2mask = self.wsi.level_dimensions[0][0] / self.wsi_mask.level_dimensions[0][0]
    #     scale_mask = self.wsi_mask.level_downsamples[seg_level]
    #     scale = (scale_img2mask * scale_mask, scale_img2mask * scale_mask)
    #     # print (scale)
    #     scaled_ref_patch_area = int(ref_patch_size ** 2 / (scale[0] * scale[1]))
    #     filter_params = filter_params.copy()
    #     filter_params['a_t'] = filter_params['a_t'] * scaled_ref_patch_area
    ...
    #
    #     # Find and filter contours
    #     contours, hierarchy = cv2.findContours(img_mask, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)  # Find contours
    #     hierarchy = np.squeeze(hierarchy, axis=(0,))[:, 2:]
    #     if filter_params: foreground_contours, hole_contours = filter_contours(contours, hierarchy,
    #                                                                            filter_params)  # Necessary for filtering out artifacts
    #
